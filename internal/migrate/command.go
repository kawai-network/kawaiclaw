...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to kawaiclaw",
		Args:  cobra.NoArgs,
		Example: `  kawaiclaw migrate
  kawaiclaw migrate --from openclaw
  kawaiclaw migrate --dry-run
  kawaiclaw migrate --refresh
  kawaiclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.kawaiclaw)")
