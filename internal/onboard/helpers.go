...
func onboard(encrypt bool) {
	...
	fmt.Printf("\n%s kawaiclaw is ready!\n", internal.Logo)
	fmt.Println("\nNext steps:")
	if encrypt {
		fmt.Println("  1. Set your encryption passphrase before starting picoclaw:")
		fmt.Println("       export KAWAICLAW_KEY_PASSPHRASE=<your-passphrase>   # Linux/macOS")
		fmt.Println("       set KAWAICLAW_KEY_PASSPHRASE=<your-passphrase>      # Windows cmd")
		fmt.Println("")
	...
	fmt.Println("")
	if encrypt {
		fmt.Println("  3. Chat: kawaiclaw agent -m \"Hello!\"")
	} else {
